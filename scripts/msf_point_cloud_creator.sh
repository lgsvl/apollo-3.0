...
function huge_point_cloud() {
  $APOLLO_BIN_PREFIX/modules/localization/msf/local_tool/map_creation/huge_point_cloud \
      --pcd_folders $1 \
      --pose_files $2
}
...
do
  ...
  huge_point_cloud "${DIR_NAME}/pcd" "${DIR_NAME}/pcd/corrected_poses.txt"
done
